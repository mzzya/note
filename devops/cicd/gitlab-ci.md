...
这种方法能够更好的处理是否需要更新缓存问题，如果不同分支依赖的包相同且很少发生变化，那么不配置`prefix`就不会进行环境隔离或许是一个更好的选择。
...
上图是个失败的演示，仅为了说明近期gitlab版本更新对阶段输出界面进行了优化，增加了计时器，可以帮助我们在遇到构建任务较慢时分析原因。
...
实测过程中在添加k8s集群时需要k8s集群的管理权限，暂时放弃验证了。但是它描述了一个非常理想化的情形，通过配置集群连接信息和少量`CI/CD`配置即可做到自动化持续集成。
