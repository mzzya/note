...
参考官方的举例，gitlab有ce社区版和ee企业版两个版本或者不同平台的安装版本，每个版本独立的构建配置文件和构建依赖，可以通过触发器来控制多个任务同时进行。

假设前后端分离的项目中在发布API项目的同时需要发布WEB项目，WEB项目依赖API项目的版本信息进行负载均衡配置，API在执行构建任务时通过触发器并传递参数（API版本信息）触发WEB的构建任务似乎是一个不错的选择。
