...
如果套用面向对象设计的单一职责原则，`gitlab-ci.yml`承担了大多的职责，所以对于不同阶段我们可以拆分成不同的文件，在需要的时候引用。官方在很早的版本就引入了组合复用，并不断完善使用体验。现在我们可以使用`include`特性引入`local`当前仓库， `file`相同gitlab，`template`官方模板和`remote`远程文件（OSS等）从不同位置引入1+个配置好的`yaml`文件进行文件复用。并使用`extends`为我们提供细致的配置代码模块复用。
