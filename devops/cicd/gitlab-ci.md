...
上图是个失败的演示，仅为了说明近期gitlab版本更新对阶段输出界面进行了优化，增加了计时器，可以帮助我们在碰到构建任务较慢时分析原因。

### trigger 触发器

参考官方的举例，gitlab有ce社区版和ee企业版两个版本或者不同平台的安装版本，每个版本独立的构建配置文件和构建依赖，可以通过触发器来控制多个任务同时进行。再者假设前后端分离的项目中在发布API项目的同时需要发布WEB项目，WEB项目依赖API项目的版本信息进行负载均衡配置，API在执行构建任务时通过触发器并传递参数（API版本信息）触发WEB的构建任务似乎是一个不错的选择。再者如果你的项目较为复杂，需要动态的生成构建任务配置文件，`GitLab 12.9`近期更新的一个版本已经支持了这种做法。同时官方提供了触发器的API，还可以将构建动作集成到别的应用当中。

### check 部署就绪检查阶段

在部署完成之后，我们通常会通过kubectl手动确认部署状态，或者是通过http服务暴露的特殊的包含版本信息的连接进行部署确认。这种做法随着项目的增多是一件很累的事，`when:delayed`延迟+`start_in`延迟多久+`retry`失败重试组合使用似乎是个很好的选择，也是我们准备补充的阶段。
...
实测过程中在添加k8s集群时需要k8s集群的管理权限，暂时放弃验证了。但是它描述了一个非常理想的情形，或需可以通过配置集群+少量配置即可做到自动化持续集成。

### Dashboard Prometheus

基于prometheus的控制面板，位于项目级别`Opeartions`->`Metrics`页面，可以配置现有的prometheus地址。像上边提到的，增加了`check`阶段仅能确认部署是否成功，业务是否能正常的运行还是需要借助一些指标进行确认。可以选择配置到这里供开发人员监控和维护项目。
