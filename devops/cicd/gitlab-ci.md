...
参考官方的举例，gitlab有ce社区版和ee企业版两个版本或者不同平台的安装版本，每个版本独立的构建配置文件和构建依赖，可以通过触发器来控制多个任务同时进行。再者假设前后端分离的项目中在发布API项目的同时需要发布WEB项目，WEB项目依赖API项目的版本信息进行负载均衡配置，API在执行构建任务时通过触发器并传递参数触发WEB的构建任务似乎是一个不错的选择。再者如果你的项目较为复杂，需要动态的创建构建任务配置文件，`GitLab 12.9`近期更新的一个版本已经支持了这种做法。同时官方提供了触发器的API，还可以将构建动作集成到别的应用当中。
