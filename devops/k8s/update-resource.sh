...
for deployName in $(kubectl get deploy -o=custom-columns=name:.metadata.name,replicas:.spec.replicas,request-cpu:.spec.template.spec.containers[0].resources.requests.cpu,limit-cpu:.spec.template.spec.containers[0].resources.limits.cpu,request-memory:.spec.template.spec.containers[0].resources.requests.memory,limit-memory:.spec.template.spec.containers[0].resources.limits.memory | grep 1000Mi | awk '{print $1}'); do
    echo $deployName
    kubectl patch deploy $deployName -p "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"${deployName}\",\"resources\":{\"requests\":{\"cpu\":\"10m\",\"memory\":\"128Mi\"},\"limits\":{\"cpu\":\"1\",\"memory\":\"1Gi\"}}}]}}}}"
done
